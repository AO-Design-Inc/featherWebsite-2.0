	...
	const teamStruct = [
		{
			name: 'Jo Gao',
			title: 'COO',
			picture: '/avatars/jo.webp'
		},
		{
			name: 'Ojasvin Kirpane',
			title: 'CEO',
			picture: '/avatars/ojas.webp'
		},
		{
			name: 'Abhishek Cherath',
			title: 'CTO',
			picture: '/avatars/abhi.webp'
		},
		{
			name: 'Kyle Li',
			title: 'CMO',
			picture: '/avatars/kyle.webp'
		}
	];
</script>
...
<div id="about-container">
	<div class="container">
		...
		<picture class="image">
			<img src="/about-illus.webp" alt="An illustration of what we do summed up abstractly." />
		</picture>
		<h1>About Us</h1>
...
<style lang="scss">
	...
	.container {
		...
		.avatar-container {
			...
			@media screen and (min-width: 1240px) {
				grid-gap: min(get-vw(60px),60px);
			}
			@media screen and (max-width: $bp) {
				grid-template-columns: 1fr;
				grid-gap: max(get-vw(60px),60px);
			}
		...
		.avatar-cards {
			...
			position: relative;
			@media screen and (min-width: 1240px) {
				width: min(get-vw(300px), 300px);
				border-radius: min(get-vw(10px), 10px);
			}
			@media screen and (max-width: $bp) {
				width: max(get-vw(300px), 100%);
				aspect-ratio: 10/10;
				overflow: hidden;
			...
			.filter {
				...
				top: 0%;
				left: 0%;
				background-image: linear-gradient(0deg, #11131C 0%, rgba(240, 173, 183, 0) 56.28%, rgba(17, 19, 28, 0) 56.28%);
				@media screen and (max-width: $bp) {
					background-image: linear-gradient(0deg, #11131ca5 0%, rgba(240, 173, 183, 0) 25%, rgba(17, 19, 28, 0) 56.28%);
				}
			}
			img {
				z-index: 1;
			...
			.name {
				...
				@media screen and (min-width: 1240px) {
					...
					max-width: min(get-vw(180px),180px);
				}
				@media screen and (max-width: $bp) {
					max-width: max(get-vw(180px),180px);
	...
	h1 {
		...
		font-size: get-vw(89px);
		text-align: center;

		@media screen and (min-width: 1240px) {
			font-size: min(get-vw(89px), 89px);
			margin-top: min(get-vw(40px), 40px);
			margin-bottom: min(get-vw(40px), 40px);
		}
		@media screen and (max-width: $bp) {
			text-align: left;
			font-size: max(get-vw(89px), 59px);
			margin-top: max(get-vw(40px), 40px);
			margin-bottom: max(get-vw(40px), 40px);
		}
	}
	h1.text {
		//gradient
		background: linear-gradient(96.56deg, #fff2f0 49.86%, #fad8d5 67.3%);
	...
	h2 {
		...
		@media screen and (min-width: 1240px) {
			font-size: min(get-vw(37px), 37px);
			margin: min(get-vw(20px), 20px) 0%;
		}
		@media screen and (max-width: $bp) {
			font-size: max(get-vw(37px), 37px);
			margin: max(get-vw(20px), 20px) 0%;
		}
	}
	p {
		font-size: 20px;
